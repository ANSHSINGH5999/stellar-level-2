...
// SDK v14: SorobanRpc moved to SDK.rpc
...
export async function getUserTotal(userAddress) {
  ...
  const result = await server.simulateTransaction(tx);
  if (StellarSDK.rpc.Api.isSimulationError(result)) {
    return 0n; // user not in storage yet — not an error
  }
...
export async function buildRecordPaymentTx(fromAddress, amountStroops, memo) {
  ...
  if (!xlmBalance || parseFloat(xlmBalance.balance) < 1) {
    throw new Error(
      ...
      `Fund your account at https://laboratory.stellar.org/#account-creator`
    );
  }

  const contract = new StellarSDK.Contract(CONTRACT_ID);
  const memoSymbol = (memo || 'payment').substring(0, 9);
  ...
  // Simulate to get auth entries + resource footprint
  const simResult = await server.simulateTransaction(tx);
  if (StellarSDK.rpc.Api.isSimulationError(simResult)) {
    throw new Error(`Contract simulation failed: ${simResult.error}`);
  }

  // Assemble injects soroban data + auth into the transaction
...
export async function submitAndPoll(signedXdr, onStatusUpdate) {
  ...
  if (sendResult.status === 'ERROR') {
    // errorResult is an XDR TransactionResult - safely stringify it
    let errDetail = 'unknown error';
    try {
      errDetail = sendResult.errorResult?.toXDR('base64') || 'unknown';
    } catch {
      errDetail = String(sendResult.errorResult || 'unknown');
    }
